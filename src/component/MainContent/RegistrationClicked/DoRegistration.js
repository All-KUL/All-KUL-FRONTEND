  ...
  useEffect(() => {
    ...
    if (enrollTime && new Date(enrollTime) <= currentTime) {
      sessionCourses.forEach((course) => {
        const totalEnrollments = course.enrollments + 10; // 총 등록 수를 100명 증가

        for (let i = 0; i < totalEnrollments; i++) {
          const randomInterval = Math.floor(Math.random() * 1000) + 1000; // Random interval between 100ms and 500ms
